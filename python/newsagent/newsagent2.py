...
def wrap(string, max=70):
	"""
	Wraps a strng to a maximum line width.
	"""
	return '\n'.join(textwrap.wrap(string)) + '\n'

class NewsAgent:
	"""
	An object that can distribute news items frm news
...
def runDefaultSetup():
	"""
	A default setup of sures and destsination. Modif to taste.
	"""
	agent = NewsAgent()

	#A simpleWebSource that retrives news from the
	#BBC News site:
