...
class EndSessions(Exception): pass
...
class CommandHandler:
	...
	def handle(self, session, line):
		...
		# Try to find a andler:
		meth = getattr(self, 'do_'+cmd, None)
		try:
			# Assume it's callable:
			meth(session, line)
		except TypeError:
			# If it isn't, respot to the unkown command:
			self.unknown(session, cmd)

class Room(CommandHandler):
	"""
	A generic environment that may contain one or more users
	(session). It takes care of basic command handling and 
	broadcasting.
	"""
...
class LoginRoom(Room):
	...
	def do_login(self, session, line):
		name = line.strip()
		# Make sure the useer has entered a name:
		if not name:
...
class ChatRoom(Room):
	"""
	A room meant for multiple users who can chat witht the others in
	the room.
	"""
	def add(self, session):
...
class ChatServer(dispatcher):
	"""
	A chat server with a single rom.
	"""
